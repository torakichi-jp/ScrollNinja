...
import org.genshin.old.scrollninja.FileOperation;
import org.genshin.old.scrollninja.GameMain;
import org.genshin.old.scrollninja.Mouse;
import org.genshin.old.scrollninja.StructObjectManager;
...
//		System.out.println("カメラY:" + GameMain.camera.position.y);
	...
	private void Move() {
		...
		if (GameMain.camera.position.x < -(BackgroundManager.backgroundList.getSprites().get(1).getWidth() * 0.5f - GlobalParam.INSTANCE.CLIENT_WIDTH * 0.5f) * GlobalParam.INSTANCE.WORLD_SCALE) {
			GameMain.camera.position.x = -(BackgroundManager.backgroundList.getSprites().get(1).getWidth() * 0.5f - GlobalParam.INSTANCE.CLIENT_WIDTH * 0.5f) * GlobalParam.INSTANCE.WORLD_SCALE;
		}
